...
from universal_mcp_contentful.app import ContentfulApp

@pytest.fixture
def app_instance():
    """Provides a ContentfulApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return ContentfulApp(integration=mock_integration)

def test_universal_mcp_contentful_app_initialization(app_instance):
    """
    Test that the ContentfulApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "contentful", "ContentfulApp instance has unexpected name."


def test_universal_mcp_contentful_tool_docstrings_format(app_instance):
    """
    Test that each tool method in ContentfulApp has a well-formatted docstring,
...
def test_universal_mcp_contentful_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in ContentfulApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
